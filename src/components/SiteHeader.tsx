    ...
    <header className="sticky top-0 z-50 bg-white border-b border-slate-200 relative">
      <div className="max-w-6xl mx-auto px-6 h-16 flex justify-between items-center">
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm text-slate-700">
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            {servicesOpen && (
              <div className="absolute left-0 top-full mt-3 w-[320px] max-w-[calc(100vw-2rem)] rounded-xl border border-slate-200 bg-white shadow-lg p-3">
                <Link href="/services" className="block rounded-lg px-3 py-2 hover:bg-slate-50" onClick={() => setServicesOpen(false)}>
                  <div className="font-medium text-slate-900">All Services</div>
                  <div className="text-xs text-slate-500">Explore our consulting pillars</div>
                </Link>

                <div className="my-2 border-t border-slate-100" />

                <Link href="/services/esg-advisory" className="block rounded-lg px-3 py-2 hover:bg-slate-50" onClick={() => setServicesOpen(false)}>
                  <div className="font-medium text-emerald-700">ESG & Sustainability</div>
                  <div className="text-xs text-slate-500">CSRD, BRSR, GHG systems, ESG data governance</div>
                </Link>

                <Link href="/services/marketing-automation" className="block rounded-lg px-3 py-2 hover:bg-slate-50" onClick={() => setServicesOpen(false)}>
                  <div className="font-medium text-indigo-700">Marketing Automation & RevOps</div>
                  <div className="text-xs text-slate-500">Lifecycle, CRM governance, automation, revenue analytics</div>
                </Link>
          ...
          <Link href="/regulatory-hub" className="hover:text-slate-900">Regulatory Hub</Link>
      ...
      {mobileOpen && (
        <div className="md:hidden bg-white border-t border-slate-200">
          <div className="max-w-6xl mx-auto px-6 py-4 flex flex-col gap-3 text-sm">
            ...
            <div className="pt-2">
              <div className="text-xs uppercase tracking-wide text-slate-500 mb-2">Services</div>
              <div className="flex flex-col gap-2">
                <Link href="/services" onClick={() => setMobileOpen(false)}>All Services</Link>
                <Link href="/services/esg-advisory" className="text-emerald-700" onClick={() => setMobileOpen(false)}>
                  ESG & Sustainability
                </Link>
                <Link href="/services/marketing-automation" className="text-indigo-700" onClick={() => setMobileOpen(false)}>
                  Marketing Automation & RevOps
                </Link>
              </div>
            </div>

            <Link href="/regulatory-hub" onClick={() => setMobileOpen(false)}>Regulatory Hub</Link>
